...
For a diagnostic draft that intentionally attempts browser smoke on a workstation, use `-AllowDraftEvidence` without `-SkipBrowser`. If local OS permissions block browser startup, the wrapper keeps the failure JSON/log, runs draft validation, and writes a blocked `candidate-summary.md`; final candidate evidence still requires a passing browser smoke and must be rerun without `-AllowDraftEvidence`.
...
Review `candidate-summary.md` first: it includes candidate git revision, working-tree state, daily workflow evidence, validator reconciliation flags, evidence validation status, warning/failure rollups, and a `Next Actions` section for the remaining acceptance work. The same output directory also includes `acceptance-checklist-draft.md` with a generated `Candidate Output Prefill` for recording the final non-production walkthrough. Then open the evidence validation JSON for the full machine-readable record. The wrapper does not authorize release, production deployment, pricing, broker execution, order routing, or autotrading.
